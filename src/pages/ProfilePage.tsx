...
const ProfilePage: React.FC = () => {
  ...
  const onUpdateProfile = async (data: any) => {
    ...
    try {
      ...
      setProfileMsg({ type: 'danger', text: err.response?.data?.detail?.[0]?.msg || 'Failed to update profile.' });
    } finally {
  ...
  const onChangePassword = async (data: any) => {
    ...
    try {
      ...
      setPasswordMsg({ type: 'danger', text: err.response?.data?.detail || 'Failed to change password.' });
    } finally {
      setLoadingPassword(false);
    }
  };

  if (!user) return <Container className="mt-5"><Alert variant="warning">Please login to view profile.</Alert></Container>;
      ...
      <Row className="mt-4">
        <Col md={4}>
          <Card className="text-center mb-4">
            <Card.Body>
              <div className="mb-3">
                <div className="rounded-circle bg-secondary d-inline-flex align-items-center justify-content-center" style={{ width: '100px', height: '100px', fontSize: '40px', color: 'white' }}>
                  ...
                  <Tab.Pane eventKey="profile">
                    ...
                    <Form onSubmit={handleProfileSubmit(onUpdateProfile)}>
                      <Form.Group className="mb-3">
                        ...
                        <Form.Control type="email" {...regProfile('email')} disabled />
                        <Form.Text className="text-muted">Email cannot be changed.</Form.Text>
                      </Form.Group>
                      <Form.Group className="mb-3">
                        <Form.Label>First Name</Form.Label>
                        <Form.Control type="text" {...regProfile('first_name')} />
                      </Form.Group>
                      <Form.Group className="mb-3">
                        <Form.Label>Last Name</Form.Label>
                        <Form.Control type="text" {...regProfile('last_name')} />
                      </Form.Group>
                  ...
                  <Tab.Pane eventKey="password">
                    ...
                    <Form onSubmit={handlePasswordSubmit(onChangePassword)}>
                      <Form.Group className="mb-3">
                        ...
                        <Form.Control type="password" {...regPassword('old_password', { required: true })} />
                      </Form.Group>
                      <Form.Group className="mb-3">
                        <Form.Label>New Password</Form.Label>
                        <Form.Control type="password" {...regPassword('new_password', { required: true, minLength: 8 })} />
                      </Form.Group>
                      <Form.Group className="mb-3">
                      ...
                      <Button variant="warning" type="submit" disabled={loadingPassword}>
